...
list<unique_ptr<Car>>::const_iterator Cars::carInFront(list<unique_ptr<Car>>::const_iterator cit, size_t lane) const {
	...
	cit++;
	while (cit != m_cars.cend() && cit->get()->getLane() != lane) cit++;
	return cit;
}
	...
	if (cit2 == m_cars.cend()) {
		...
		if (!freeSight(it, maxDist, position.m_lane, true, dist)) {
			if (position.m_lane == 0) {
				...
				if (freeSight(it, maxDist, 1, true, freeDist) &&
					freeSight(it, speed*m_sim.m_freeSight, 1, false, freeDistBehind) &&
					!m_fw.roadWorksAhead(position1, speed)) {
					//change lane
					car.changeLane(1);
					car.addDist(maxDist);
					return false;
